...
/*
Write a function called extractFullName which accepts an array of objects and returns a new array with
 the value of the key with a name of "first" and the value of a key with the name of  "last" in each 
...
function findInObj(arr, key, searchValue) {
    let obj = undefined; 
    arr.filter(b => {
        if(b[key] === searchValue && obj === undefined){
            obj = b; 
        }
    });    
    return obj; 
...
function removeVowels(str) {
    ...
    str = str.toLowerCase(); 
    str = Array.from(str);
    return str.filter(b => !vowels.includes(b)).join(''); 
}

/*
Write a function called doubleOddNumbers which accepts an array and returns a 
new array with all of the odd numbers doubled (HINT - you can use map and filter to
...
function doubleOddNumbers(arr) {
    return arr.filter(b => b % 2 !== 0).map(b => b * 2);
}
